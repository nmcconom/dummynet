...
static struct _s_x dummynet_params[] = {
        ...
        { "pipe",               TOK_PIPE },
        { "queue",              TOK_QUEUE },
        { "queue-delay",        TOK_QUEUE_DELAY },
        { "queue-increments", TOK_QUEUE_INCREMENTS },
        { "flowset",            TOK_FLOWSET },
        { "sched",              TOK_SCHED },
        { "pri",                TOK_PRI },
        { "priority",           TOK_PRI },
...
list_pipes(struct dn_id *oid, struct dn_id *end)
{
    ...
    for (; oid != end; oid = O_NEXT(oid, oid->len)) {
        ...
        case DN_LINK: {
            ...
            sprintf(buf, "%05d: %s delay %d ms jitter %4dms queue-delay %4dms queue-increments %3d burst %s",
                p->link_nr % DN_MAX_ID, bwbuf, p->delay, p->jitter, p->queue_delay, p->queue_increments, burst);
            }
            break;
...
#define ED_TOK_QUEUE_DELAY   "queue-delay"
#define ED_TOK_QUEUE_INCREMENTS  "queue-increments"
        ...
        while (fgets(line, ED_MAX_LINE_LEN, f)) {        /* read commands */
                ...
                if (!strcasecmp(name, ED_TOK_SAMPLES)) {
                    ...
                    if (do_points)
                        errx(ED_EFMT("duplicated token: %s"), name);
                    do_points = 0;
                } else if (!strcasecmp(name, ED_TOK_QUEUE_DELAY)) {
                    if (do_points)
                        errx(ED_EFMT("duplicated token: %s"), name);
                    do_points = 0;
                } else if (!strcasecmp(name, ED_TOK_QUEUE_INCREMENTS)) {
                    if (do_points)
                        errx(ED_EFMT("duplicated token: %s"), name);
                    do_points = 0;
...
end_mask:
                ...
                case TOK_QUEUE_DELAY:
                        NEED(p, "queue-delay is only for links");
                        NEED1("queue-delay needs argument 0..10000ms\n");
                        p->queue_delay = strtoul(av[0], NULL, 0);
                        ac--; av++;
                        break;

                case TOK_QUEUE_INCREMENTS:
                        NEED(p, "queue-increments is only for links");
                        NEED1("queue-increments needs argument 0..100\n");
                        p->queue_increments = strtoul(av[0], NULL, 0);
                        ac--; av++;
                        break;
